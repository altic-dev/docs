...
- **All apps** — the selected prompt profile is used as the global default for all apps, with per-app bindings overriding it for specific apps.
- **Selected apps only** — the selected prompt profile is only active for apps that have an explicit binding. All other apps use the built-in default prompt.

Set the routing scope in **Settings → Prompts** for each mode independently.

## How App Detection Works

FluidVoice uses macOS's accessibility API to monitor which application is currently focused. When you press your dictation hotkey, FluidVoice reads the active app's bundle identifier and looks up any matching prompt binding for the current mode. The matching profile's instructions are sent to the AI provider as part of the system prompt — no manual selection required on your part.
