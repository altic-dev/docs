...
<CardGroup cols={2}>
  <Card title="Fluid Intelligence" icon="microchip" href="/features/fluid-intelligence">
    **On-device · No API key · ~3.5 GB download**

    Runs the Fluid-1 model locally on your Mac. No data leaves your device. Requires Apple Silicon.
  </Card>
  <Card title="OpenAI" icon="robot" href="/features/ai-enhancement">
    **Cloud · API key required**

    Uses OpenAI's models (GPT-4o, GPT-4.1, and others) for high-quality enhancement. Transcripts are sent to OpenAI's servers.
  </Card>
  <Card title="Groq" icon="bolt" href="/features/ai-enhancement">
    **Cloud · API key required**

    Fast inference via Groq's hardware. Good balance of speed and quality for real-time dictation enhancement.
  </Card>
  <Card title="Custom Provider" icon="server" href="/features/ai-enhancement">
    **Any OpenAI-compatible endpoint**

    Point FluidVoice at any server that speaks the OpenAI chat completions API — Ollama, LM Studio, self-hosted models, or enterprise endpoints.
  </Card>
</CardGroup>

<Tip>
  Use **Fluid Intelligence** if you want fully private AI enhancement with no API key, no cost, and no data leaving your Mac. All processing happens on-device, including when you're offline.
</Tip>
...
<Steps>
  ...
  <Step title="Select your provider">
    Choose **OpenAI**, **Groq**, or **Custom** from the provider list. If you want Fluid Intelligence, select it and follow the model download prompt instead.
  </Step>
...
<Steps>
  <Step title="Add a custom provider">
    In **Settings → AI Enhancement**, select **Custom** and click **Add Provider**.
  </Step>
...
FluidVoice applies AI Enhancement separately for **Dictate mode** and **Edit mode**. This means you can, for example, use Fluid Intelligence for standard dictation while using OpenAI for text rewrites — or disable enhancement entirely for one mode while keeping it active for the other.
...
To turn off AI Enhancement entirely, go to **Settings → AI Enhancement** and set the provider to **Off**. FluidVoice falls back to Direct Dictation: the raw transcript is typed into the active app without any post-processing.
