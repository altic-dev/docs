...
| **Final text** | The post-processed result that was typed into the app |
| **AI processed** | Whether an AI enhancement provider was applied |

Having both the raw and final text in the same entry makes it easy to see what the speech model heard versus what the AI changed, which is useful for tuning your prompts and spotting persistent transcription errors.
...
The history view shows your recent dictations in reverse chronological order. Select any entry to expand it and see the full raw and final text side by side.
...
```bash
# Get the last 20 history entries (default limit)
...
Each response item includes `id`, `timestamp`, `originalText`, `finalText`, `appName`, `characterCount`, and `wasAIProcessed`.
